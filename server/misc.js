...
module.exports = {
		...
		if (renderProps.location.pathname.indexOf("/video/") === 0) {
			global.videos
				...
				.next(function(err, video) {
					...
					head.type = "article";
					head.url =
						"https://sailing-channels.com/video/" + video._id;
			...
			global.channels
				...
				.next(function(err, channel) {
					...
					head.type = "profile";
					head.url =
						"https://sailing-channels.com/channel/" +
	...
	languages: function(req, res) {
		...
		global.CACHE_languages.findOne({ _id: "languages" }, function(
			err,
			data
		) {
			// not in cache
			if (err || !data) {
				...
				global.channels.distinct("language", function(err, languages) {
					...
					languages["selected"] =
						req.cookies["channel-language"] || "en";
	...
	getYouTubeChannelId: function(url, callback) {
		...
		request.get("https://youtube.com/user/" + ytUserOrChannel, function(
			err,
			response,
			body
		) {
