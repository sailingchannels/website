								...
								function(err, result) {
									...
									if (result.user) {
										if (result.user.profile) {
											channel.boatcolor =
												result.user.profile.boatcolor;
											channel.trailnumber =
												result.user.profile.trailnumber;
										} else {
											//channel.boatcolor = ""
										}
		...
		global.CACHE_custom_links.findOne(
			...
			function(err, data) {
				if (!err && data) {
					...
					youtubeCustomLinks.customLinks(req.params.id, function(
						err,
						links
					) {
						if (err) {
		...
		youtube.subscriptions.list(
			...
			function(err, subs) {
				...
				if (subs.pageInfo.totalResults !== 1) {
					return res
						.status(500)
						.send({ error: "could not find subscription result" });
				}
