...
module.exports = {
	...
	readInReachPosition: function(userId, username, callback) {
		...
		// try cache
		global.CACHE_inreach_positions
			.find({ _id: username })
			.next(function(err, data) {
				// not a cache hit
				if (err || !data) {
	...
	readPosition: function(user, callback) {
		...
		if (profile && "inreach" in profile && profile.inreach.length > 0) {
			module.exports.readInReachPosition(
				user._id,
				profile.inreach,
				function(pos, more) {
					...
					if (pos === null) {
						if (
							profile &&
							"mmsi" in profile &&
							profile.mmsi.length > 0
						) {
							module.exports.readAISPosition(
								user._id,
								profile.mmsi,
								function(pos, more) {
			...
			// check for mmsi
			module.exports.readAISPosition(user._id, profile.mmsi, function(
				pos,
				more
			) {
				return callback(pos, more, "mmsi");
			...
			global.positions
				...
				.toArray(function(err, positions) {
					...
					if (err || !positions) {
						return res
							.status(500)
							.send(err || "unable to retrieve positions");
					}
		...
		global.videos
			...
			.toArray(function(err, locations) {
				...
				if (err || !locations) {
					return res
						.status(500)
						.send(err || "unable to retrieve geolocations");
				}
